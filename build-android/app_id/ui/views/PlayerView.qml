...
import "../components"
// Nếu folder chứa VideoPlayerView.qml khác thì mày nhớ import,
// nhưng nãy tao thấy mày để chung thư mục views nên chắc đéo cần thêm import.
...
Item {
    ...
    // ================= SỰ KIỆN ĐỔI BÀI HÁT (CHÌA KHÓA Ở ĐÂY) =================
    onCurrentTrackChanged: {
        if (root.currentTrack && root.currentTrack.isVideo) {
            console.log(">>> PlayerView bung lụa Video MP4!");
            videoUI.playVideo(root.currentTrack.filePath);
        } else {
            console.log(">>> PlayerView dọn Video, quay lại đĩa than MP3!");
            videoUI.stopVideo();
        }
    }

    // ================= MẶT A: GIAO DIỆN ĐĨA THAN (DÀNH CHO MP3) =================
    Item {
        id: audioUI
        anchors.fill: parent

        // ẨN ĐI NẾU BÀI HIỆN TẠI LÀ VIDEO!
        visible: !root.currentTrack || !root.currentTrack.isVideo
    ...
    // ================= MẶT B: GIAO DIỆN VIDEO (DÀNH CHO MP4) =================
    VideoPlayerView {
        id: videoUI
        anchors.fill: parent
        z: 99 // Đẩy nó nổi lên trên cùng để che lấp mọi thứ

        // CHỈ HIỆN LÊN NẾU BÀI HIỆN TẠI LÀ VIDEO!
        visible: root.currentTrack && root.currentTrack.isVideo
    }
}